-- Migration: Fix shipment address to copy from user data
-- Date: 2026-03-27
-- Description: Update confirm_assign_sets_to_users to populate shipping_address, 
-- shipping_city, shipping_zip_code, shipping_province from user table data
-- in addition to PUDO information
...
BEGIN
    ...
    FOR r IN (
        ...
        IF target_set_id IS NOT NULL THEN
            ...
            -- Create shipment with USER address data, not PUDO
            -- Use user's own address as shipping address
...
'Confirms set assignments to users. Populates shipment address fields from user data (address, city, zip_code, province) instead of PUDO location. This ensures shipping address is always available even when PUDO location details are incomplete.';