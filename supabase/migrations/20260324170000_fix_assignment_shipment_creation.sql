-- Migration: Fix assignment to properly set user_status to 'set_shipping' and shipment_status to 'assigned'
...
COMMENT ON FUNCTION public.confirm_assign_sets_to_users IS 'Confirms set assignments for users. Sets user_status to set_shipping, creates shipment with assigned status, and copies PUDO address data to shipment record. Requires PUDO point configured.';