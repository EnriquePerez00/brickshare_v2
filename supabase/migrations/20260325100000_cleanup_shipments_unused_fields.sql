-- Migration: Remove unused shipment date fields and populate assigned_date
-- Date: 2026-03-25
-- Impact: Drops 4 unused columns (estimated_delivery_date, warehouse_pickup_date, 
--         estimated_return_date, last_tracking_update) and updates assignment function
--         to populate assigned_date when sets are assigned to users.

-- 1. Drop unused date tracking columns that were never populated
ALTER TABLE public.shipments 
DROP COLUMN IF EXISTS estimated_delivery_date,
DROP COLUMN IF EXISTS warehouse_pickup_date,
DROP COLUMN IF EXISTS estimated_return_date,
DROP COLUMN IF EXISTS last_tracking_update;

-- 2. Update confirm_assign function to populate assigned_date
...
BEGIN
    ...
    FOR r IN (
        ...
        IF target_set_id IS NOT NULL THEN
            ...
            -- Create shipment with PUDO address data, 'assigned' status, and assigned_date
...
COMMENT ON FUNCTION public.confirm_assign_sets_to_users IS 'Confirms set assignments for users. Sets user_status to set_shipping, creates shipment with assigned status and assigned_date, and copies PUDO address data to shipment record. Requires PUDO point configured.';