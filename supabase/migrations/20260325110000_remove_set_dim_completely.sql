-- Migration: Remove all set_dim references from confirm_assign_sets_to_users
-- Date: 2026-03-25
-- Issue: Function tries to SELECT s.set_dim which doesn't exist in sets table

-- Drop and recreate function without set_dim
...
BEGIN
    ...
    FOR r IN (
        ...
        -- Find the first available set from user's wishlist (without set_dim)
        SELECT 
            w.set_id,
            s.set_name,
            s.set_ref,
        ...
        IF target_set_id IS NOT NULL THEN
            ...
            -- Populate return record (without set_dim)
