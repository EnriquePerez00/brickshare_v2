...
const AuthForm = ({ 
  ...
  const [mode, setMode] = useState<"login" | "signup" | "forgot-password" | "update-password">(initialMode);
  ...
  const { signIn, signUp, signInWithGoogle, resetPassword, updateUserPassword, user, isAdmin, isOperador } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "login") {
      ...
      // NOTE: fullName is no longer requested here, collected in ProfileCompletionModal
      ...
      if (error) {
        ...
        toast({
          title: "¡Cuenta creada!",
          description: "Has iniciado sesión correctamente. Por favor completa tu perfil.",
        });
        redirectBasedOnRole();
      }
    ...
    <div className="w-full">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground">
          {mode === "login" && "Accede a tu cuenta de Brickshare"}
